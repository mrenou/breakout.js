TestCase("BarTest", {
    gameArea:new GameArea(1000, 1000, {}),
    COORDINATE_X:460,
    COORDINATE_Y:990,
    "setUp":function () {
    ...
    "test : bar should return right bound coordinate x":function () {
        ...
        // Then
        assertEquals(this.COORDINATE_X + Bar.WIDTH, rightBoundCoordinateX);
    },
    ...
    "test : bar should return bottom bound coordinate y":function () {
        ...
        // Then
        assertEquals(this.COORDINATE_Y + Bar.HEIGHT, bottomBoundCoordinateY);
    },
    ...
    "test : bar should move to right":function () {
        ...
        // Then
        assertEquals(this.COORDINATE_X + Bar.SPEED, bar.coordinates.x);
        assertEquals(this.COORDINATE_Y, bar.coordinates.y);
    },
    "test : bar should not move to right when is at right bound":function () {
        // Given
        var bar = new Bar(this.gameArea);
        bar.coordinates.x = this.gameArea.width - Bar.WIDTH;
        ...
        // Then
        assertEquals(this.gameArea.width - Bar.WIDTH, bar.coordinates.x);
        assertEquals(this.COORDINATE_Y, bar.coordinates.y);
    },
    ...
    "test : bar should draw":function () {
        ...
        // Then
        assertEquals(Bar.COLOR, contextMocked.fillStyle);
        verify(contextMocked).fillRect(this.COORDINATE_X, this.COORDINATE_Y, Bar.WIDTH, Bar.HEIGHT);
    },
    "test : bar should init":function () {
        ...
        // Then                                                         ,
        assertEquals((this.gameArea.width / 2) - (Bar.WIDTH / 2), bar.coordinates.x);
        assertEquals(this.gameArea.height - Bar.HEIGHT, bar.coordinates.y);
    },
});
